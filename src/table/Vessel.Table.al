...
table 50252 "Vessel"
{
    ...
    fields
    {
        ...
        {
            Caption = 'Vessel'; //Commande
            DataClassification = ToBeClassified;
        }
        field(2; Port; Text[30])
        {
            Caption = 'Port'; //Commande
            DataClassification = ToBeClassified;
        }
        field(3; "IMO No."; Text[100])
        {
            Caption = 'IMO No.'; //Commande
            DataClassification = ToBeClassified;
        }
        field(4; ETD; Date)
        {
        ...
        {
            Caption = 'Items Category'; //Lignes Commande
            DataClassification = ToBeClassified;
        }
        field(8; Comments; Text[250])
        {
            Caption = 'Comments'; //Utilisateur
            DataClassification = ToBeClassified;
        }
        field(9; "Delivery Date"; Date)
        {
            Caption = 'Delivery Date'; //Commande
            DataClassification = ToBeClassified;
        }
        ...
        {
            Caption = 'Sales order No.'; //Commande
            DataClassification = ToBeClassified;
        }
    ...
    /// ArchiveVessel.
    /// </summary>
    procedure ArchiveVessel()
    var
        VesselArchiveRecord: Record VesselArchive;
        MarineTrafficSetup: Record MarineTrafficSetup;
        MarineTrafficAPI: Codeunit "Marine Traffic API";
        ConfirmLbl: Label 'Vessel (IMO %1) correctly deleted from fleet.', Comment = 'IMO No', Locked = true;
    begin
        if not VesselArchiveRecord.Get(Rec.Vessel, Rec.Port) then begin
            VesselArchiveRecord.Init();
            VesselArchiveRecord.TransferFields(Rec, true);
            if VesselArchiveRecord.Insert(true) then begin
                MarineTrafficSetup.Get();
                MarineTrafficAPI.VesselToFleet(Rec."IMO No.", MarineTrafficSetup.FleetID, '1', 1); //Delete Vessel from fleet
                Rec.Delete(true);
